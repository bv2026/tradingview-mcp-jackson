...
- For `futures` specifically, the SIGNAL cell in that table should include the literal numeric histogram/signal values in the form `Hist ±X.XX ... sig ±Y.YY` (e.g. `Hist +174.83 still positive, just below sig 293.58 — decelerating`) rather than qualitative-only phrasing like `hist+ but decelerating hard` — the reader needs the numbers to judge deceleration.
- **`futures` SYMBOL column: bare ticker only** — e.g. `ES1!`, never `ES1! (S&P)` or any other descriptive suffix (keeps the table scannable and consistent with the other briefs).
- **`futures` negative numbers: use the ASCII hyphen-minus `-` (U+002D)** in Hist/sig/gap values, not the typographic minus `−` (U+2212) or an en/em dash.
- **`futures` keeps the literal `**Benchmark:**` / `**Theme:**` headers** from the generic convention above even though futures also carries macro-overlay content (DXY/bonds/VX1! context) — put that content as bullets *under* `**Theme:**` (or its own clearly-separate block before it), don't rename `**Theme:**`.
...
**Routine discoverability + versioning:** check `scripts/scheduled-tasks/VERSIONS.md` before hand-rolling any email/report output — it maps every routine's canonical copy, live scheduler copy, and (where one exists) manually-invocable project skill under `.claude/skills/<name>/`, plus a `version`/`last_updated` for each. A routine worth running on demand (not only at its cron time) should get a project skill: prefer a **modular** skill — a slim `SKILL.md` orchestrator plus a `references/` folder of small, independently-editable files loaded only as each step is reached — over one large monolithic file. `.claude/skills/weekly-decision-routine/` is the reference implementation of this pattern. Bump `version` in both the canonical copy and the project skill together whenever behavior changes, and log the change in `VERSIONS.md`'s changelog.
