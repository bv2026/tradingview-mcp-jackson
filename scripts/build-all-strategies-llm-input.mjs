...
const watchLimit = Math.max(0, Number(process.env.LLM_WATCH_LIMIT || 5));
...
for (const family of families) {
  ...
  const watch = all.filter(r => r.status === 'WATCH').sort((a,b) => (b.rank_score ?? -Infinity) - (a.rank_score ?? -Infinity));
  const refs = watch.slice(watchLimit).map(r => ({ strategy: family, symbol: r.symbol, full_symbol: r.full_symbol, score: r.score, rank_score: r.rank_score, eligibility: r.eligibility, evidence_artifact: relative(root, p).replaceAll('\\','/'), row_locator: { key: r.symbol || r.full_symbol } }));
  strategies[family] = { summary: summarize(rows, d, p), candidates: [...review, ...watch.slice(0, watchLimit)], watch_references: refs, selection: { review_included: review.length, watch_included: Math.min(watch.length, watchLimit), watch_references: refs.length, remaining_watch: refs.length, remaining_rows: Math.max(0, all.length-review.length-watch.length), total_rows: all.length } };
}
...
pkg.retrieval_policy = { watch_limit: watchLimit, included: 'all REVIEW plus top WATCH by descending rank_score', omitted_watch: 'deterministic references in watch_references_by_strategy; hydrate from evidence_artifact and row_locator', freshness_threshold_hours: freshnessHours };
pkg.watch_references_by_strategy = Object.fromEntries(Object.entries(strategies).map(([k,v]) => [k, v.watch_references]));
