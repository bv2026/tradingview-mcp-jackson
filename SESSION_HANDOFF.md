...
### Other Scripts (present but not primary)

- `scripts/futures_decision_hook.py` — PostToolUse hook prototype, abandoned (hooks in scheduled task context have no Claude to act on output)
- `scripts/decision_engine.py` — old pre-classification approach, superseded by ct_tv_data.py + inline LLM reasoning; do not use
...
- `scripts/futures_decision_hook.py`
